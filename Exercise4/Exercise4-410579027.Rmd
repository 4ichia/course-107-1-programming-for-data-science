...
```{r ans1}
sum(gsHW2ws$ans1 ==1 , na.rm=TRUE)/nrow(gsHW2ws)
```
...
```{r ans12}
library(stringr)
str_which(colnames(gsHW2ws),c("ans"))
```
...
allBankData$`年月`<- as.Date(allBankData$`年月`)
```
...
#exData %>% mutate_at(
  #vars(exData$`英鎊USD/GBP`,exData$`澳幣USD/AUD`,exData$`歐元USD/EUR`),
  #funs(1/.)
#) -> exData
```
