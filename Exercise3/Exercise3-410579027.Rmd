...
```{r ans11}
library(stringr)
classData$`公-10-06` <- str_replace(str_replace(classData$`公-10-06`,"1","有看公告"),"0","沒看公告")
...
```{r ans12}
classData$`公-10-06`[classData$`公-10-06`== "有看公告"]
```
...
```{r ans13}
a <- classData$`公-10-06`[classData$`練2018-10-17-1`== "1"]
a[a == "有看公告"]
```
...
```{r ans21}
library(lubridate)
with_tz(mdy_hms(hwData$Timestamp), "Asia/Taipei")
```
...
```{r ans22}
with_tz(deadline <- ymd_hms(c("2018-10-23 16:00:00")),"Asia/Taipei")
```
...
```{r ans23}
difftime(deadline - hwData$Timestamp)
```
