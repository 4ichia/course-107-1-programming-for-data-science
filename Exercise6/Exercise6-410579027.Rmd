...
#     性別=as.factor(`性別`),
#     入學年=str_sub(學號,2,4),
#     團隊活動 = !(課外活動 == "無") #這樣打工也算進去了，但題目只有要團隊活動的
#   )->classSurvey
...
#     性別=as.factor(`性別`),
#     入學年=str_sub(學號,2,4),
#     團隊活動 = (課外活動 == "團隊活動") #這樣有團隊活動但也有打工的人會被排除，因為使用==就是要一模一樣
#   )->classSurvey
...
classSurvey %>%
  mutate(
    性別=as.factor(性別),
    入學年=str_sub(學號,2,4),
    團隊活動=str_detect(課外活動,"團隊活動")  #str_detect使用
...
#各種嘗試
#str_replace_na(x==NA,"NA")
#x[is.na(x)] <- 0
#X=as.character(x)
# str_replace(x,NA,"0")
 # ifelse(x==is.na(x),O,x)
 # str_replace(x,x==is.na(x),0)
```
